...
func process_transforms(transforms: TransformList, ScatterDomain, global_seed: int) -> void:
	if not ScatterDomain.get_root().is_inside_tree():
		return
	...
	if Engine.is_editor_hint():
		...
		if ScatterDomain.is_empty() and not warning_ignore_no_shape:
	...
	await _process_transforms(transforms, ScatterDomain, random_seed)
...
func _process_transforms(_transforms: TransformList, _ScatterDomain: ScatterDomain, _seed: int) -> void:
	pass
